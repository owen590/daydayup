import { LearningItem } from './types';

// 一年级下册英文单词表 (60个单词)
export const ENGLISH_GRADE_1B: LearningItem[] = [
  // 单元1: 植物与动物(Plants and Animals)
  {
    id: 'en-1B-1-01',
    term: 'flower',
    pronunciation: '/ˈflaʊər/',
    meaning: '花',
    example: 'The flower is beautiful.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '植物',
    isWritable: true
  },
  {
    id: 'en-1B-1-02',
    term: 'tree',
    pronunciation: '/triː/',
    meaning: '树',
    example: 'The tree is tall.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '植物',
    isWritable: true
  },
  {
    id: 'en-1B-1-03',
    term: 'grass',
    pronunciation: '/ɡræs/',
    meaning: '草',
    example: 'The grass is green.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '植物',
    isWritable: false
  },
  {
    id: 'en-1B-1-04',
    term: 'bird',
    pronunciation: '/bɜːrd/',
    meaning: '鸟',
    example: 'The bird can fly.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: true
  },
  {
    id: 'en-1B-1-05',
    term: 'fish',
    pronunciation: '/fɪʃ/',
    meaning: '鱼',
    example: 'The fish swims in water.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: true
  },
  {
    id: 'en-1B-1-06',
    term: 'cat',
    pronunciation: '/kæt/',
    meaning: '猫',
    example: 'The cat is cute.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: true
  },
  {
    id: 'en-1B-1-07',
    term: 'dog',
    pronunciation: '/dɔːɡ/',
    meaning: '狗',
    example: 'The dog is friendly.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: true
  },
  {
    id: 'en-1B-1-08',
    term: 'butterfly',
    pronunciation: '/ˈbʌtərflaɪ/',
    meaning: '蝴蝶',
    example: 'The butterfly is colorful.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: false
  },
  {
    id: 'en-1B-1-09',
    term: 'bee',
    pronunciation: '/biː/',
    meaning: '蜜蜂',
    example: 'The bee makes honey.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: false
  },
  {
    id: 'en-1B-1-10',
    term: 'rabbit',
    pronunciation: '/ˈræbɪt/',
    meaning: '兔子',
    example: 'The rabbit has long ears.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: false
  },
  {
    id: 'en-1B-1-11',
    term: 'duck',
    pronunciation: '/dʌk/',
    meaning: '鸭子',
    example: 'The duck quacks.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: true
  },
  {
    id: 'en-1B-1-12',
    term: 'chicken',
    pronunciation: '/ˈtʃɪkɪn/',
    meaning: '鸡',
    example: 'The chicken lays eggs.',
    category: 'english',
    grade: '一年级下',
    unit: '第一单元',
    subcategory: '动物',
    isWritable: false
  },

  // 单元2: 日常活动(Daily Activities)
  {
    id: 'en-1B-2-01',
    term: 'eat',
    pronunciation: '/iːt/',
    meaning: '吃',
    example: 'I eat apples.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-02',
    term: 'drink',
    pronunciation: '/drɪŋk/',
    meaning: '喝',
    example: 'I drink water.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-03',
    term: 'sleep',
    pronunciation: '/sliːp/',
    meaning: '睡觉',
    example: 'I sleep at night.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-04',
    term: 'walk',
    pronunciation: '/wɔːk/',
    meaning: '走路',
    example: 'I walk to school.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-05',
    term: 'run',
    pronunciation: '/rʌn/',
    meaning: '跑',
    example: 'I run fast.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-06',
    term: 'jump',
    pronunciation: '/dʒʌmp/',
    meaning: '跳',
    example: 'I can jump high.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-07',
    term: 'play',
    pronunciation: '/pleɪ/',
    meaning: '玩',
    example: 'I play with toys.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-08',
    term: 'sing',
    pronunciation: '/sɪŋ/',
    meaning: '唱歌',
    example: 'I sing a song.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: false
  },
  {
    id: 'en-1B-2-09',
    term: 'draw',
    pronunciation: '/drɔː/',
    meaning: '画画',
    example: 'I draw a cat.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-10',
    term: 'write',
    pronunciation: '/raɪt/',
    meaning: '写',
    example: 'I write my name.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-11',
    term: 'read',
    pronunciation: '/riːd/',
    meaning: '读',
    example: 'I read a book.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: true
  },
  {
    id: 'en-1B-2-12',
    term: 'listen',
    pronunciation: '/ˈlɪsn/',
    meaning: '听',
    example: 'I listen to music.',
    category: 'english',
    grade: '一年级下',
    unit: '第二单元',
    subcategory: '动作',
    isWritable: false
  },

  // 单元3: 食物与饮料(Food and Drinks)
  {
    id: 'en-1B-3-01',
    term: 'apple',
    pronunciation: '/ˈæpl/',
    meaning: '苹果',
    example: 'I eat an apple.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '水果',
    isWritable: true
  },
  {
    id: 'en-1B-3-02',
    term: 'banana',
    pronunciation: '/bəˈnɑːnə/',
    meaning: '香蕉',
    example: 'I like bananas.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '水果',
    isWritable: true
  },
  {
    id: 'en-1B-3-03',
    term: 'orange',
    pronunciation: '/ˈɔːrɪndʒ/',
    meaning: '橙子',
    example: 'The orange is sweet.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '水果',
    isWritable: true
  },
  {
    id: 'en-1B-3-04',
    term: 'grape',
    pronunciation: '/ɡreɪp/',
    meaning: '葡萄',
    example: 'Grapes are purple.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '水果',
    isWritable: false
  },
  {
    id: 'en-1B-3-05',
    term: 'pear',
    pronunciation: '/per/',
    meaning: '梨',
    example: 'The pear is juicy.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '水果',
    isWritable: false
  },
  {
    id: 'en-1B-3-06',
    term: 'milk',
    pronunciation: '/mɪlk/',
    meaning: '牛奶',
    example: 'I drink milk every day.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '饮料',
    isWritable: true
  },
  {
    id: 'en-1B-3-07',
    term: 'water',
    pronunciation: '/ˈwɔːtər/',
    meaning: '水',
    example: 'Water is good for you.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '饮料',
    isWritable: true
  },
  {
    id: 'en-1B-3-08',
    term: 'juice',
    pronunciation: '/dʒuːs/',
    meaning: '果汁',
    example: 'I like orange juice.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '饮料',
    isWritable: false
  },
  {
    id: 'en-1B-3-09',
    term: 'bread',
    pronunciation: '/bred/',
    meaning: '面包',
    example: 'I eat bread for breakfast.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '主食',
    isWritable: true
  },
  {
    id: 'en-1B-3-10',
    term: 'rice',
    pronunciation: '/raɪs/',
    meaning: '米饭',
    example: 'I eat rice for dinner.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '主食',
    isWritable: true
  },
  {
    id: 'en-1B-3-11',
    term: 'egg',
    pronunciation: '/eɡ/',
    meaning: '鸡蛋',
    example: 'I eat an egg for breakfast.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '食物',
    isWritable: true
  },
  {
    id: 'en-1B-3-12',
    term: 'cake',
    pronunciation: '/keɪk/',
    meaning: '蛋糕',
    example: 'I eat cake on my birthday.',
    category: 'english',
    grade: '一年级下',
    unit: '第三单元',
    subcategory: '甜点',
    isWritable: true
  },

  // 单元4: 衣服与天气(Clothes and Weather)
  {
    id: 'en-1B-4-01',
    term: 'shirt',
    pronunciation: '/ʃɜːrt/',
    meaning: '衬衫',
    example: 'I wear a red shirt.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '衣服',
    isWritable: false
  },
  {
    id: 'en-1B-4-02',
    term: 'pants',
    pronunciation: '/pænts/',
    meaning: '裤子',
    example: 'I wear blue pants.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '衣服',
    isWritable: false
  },
  {
    id: 'en-1B-4-03',
    term: 'skirt',
    pronunciation: '/skɜːrt/',
    meaning: '裙子',
    example: 'She wears a pink skirt.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '衣服',
    isWritable: false
  },
  {
    id: 'en-1B-4-04',
    term: 'dress',
    pronunciation: '/dres/',
    meaning: '连衣裙',
    example: 'She wears a pretty dress.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '衣服',
    isWritable: false
  },
  {
    id: 'en-1B-4-05',
    term: 'shoe',
    pronunciation: '/ʃuː/',
    meaning: '鞋',
    example: 'I wear new shoes.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '衣服',
    isWritable: true
  },
  {
    id: 'en-1B-4-06',
    term: 'sock',
    pronunciation: '/sɑːk/',
    meaning: '袜子',
    example: 'I wear white socks.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '衣服',
    isWritable: false
  },
  {
    id: 'en-1B-4-07',
    term: 'coat',
    pronunciation: '/koʊt/',
    meaning: '外套',
    example: 'I wear a coat in winter.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '衣服',
    isWritable: false
  },
  {
    id: 'en-1B-4-08',
    term: 'hat',
    pronunciation: '/hæt/',
    meaning: '帽子',
    example: 'I wear a hat in summer.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '衣服',
    isWritable: false
  },
  {
    id: 'en-1B-4-09',
    term: 'sunny',
    pronunciation: '/ˈsʌni/',
    meaning: '晴朗的',
    example: 'Today is sunny.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '天气',
    isWritable: false
  },
  {
    id: 'en-1B-4-10',
    term: 'rainy',
    pronunciation: '/ˈreɪni/',
    meaning: '下雨的',
    example: 'It is rainy today.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '天气',
    isWritable: false
  },
  {
    id: 'en-1B-4-11',
    term: 'windy',
    pronunciation: '/ˈwɪndi/',
    meaning: '有风的',
    example: 'It is windy today.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '天气',
    isWritable: false
  },
  {
    id: 'en-1B-4-12',
    term: 'snowy',
    pronunciation: '/ˈsnoʊi/',
    meaning: '下雪的',
    example: 'It is snowy in winter.',
    category: 'english',
    grade: '一年级下',
    unit: '第四单元',
    subcategory: '天气',
    isWritable: false
  }
];
